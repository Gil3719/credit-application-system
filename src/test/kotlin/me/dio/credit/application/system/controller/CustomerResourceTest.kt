...
class CustomerResourceTest {
    ...
    fun `should create a customer and return 201 status`(){
        ...
        val valueAssString: String = objectMapper.writeValueAsString(customerDto)
        //when
        //then
        mockMvc.perform(MockMvcRequestBuilders.post(URl).contentType(MediaType.APPLICATION_JSON)
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("email@teste.com"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.income").value("1000.0"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.zipCode").value("cep"))
    ...
    @Test
    fun `should not save a customer whith the same CPF and return 409 status`(){
        //given
    ...
    fun `should not save a customer with empty firstName and return 400 status`() {
        ...
        //then
        mockMvc.perform(
            MockMvcRequestBuilders.post(URl).content(valueAsString).contentType(MediaType.APPLICATION_JSON))
    ...
    fun `should update a customer and return 200 status`() {
        ...
        mockMvc.perform(MockMvcRequestBuilders.patch("$URl?customerId=${customer.id}")
            ...
            .andExpect(MockMvcResultMatchers.status().isOk)
            .andExpect(MockMvcResultMatchers.jsonPath("$.firstName").value("Gil2"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.lastName").value("teste2"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.cpf").value("28475934625"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("email@teste.com"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.income").value("5000.0"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.zipCode").value("cep2"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.street").value("rua2"))
            //.andExpect(MockMvcResultMatchers.jsonPath("$.id").value(1))
            .andDo(MockMvcResultHandlers.print())
    }
    ...
    private fun builderCustomerUpdateDto(
        firstName: String = "Gil2",
        lastName: String = "teste2",
